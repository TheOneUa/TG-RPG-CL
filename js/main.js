...
// ============================================================
// Вынесено в ui/CharScreen/{CharScreen,StatsTab,EquipTab,InventoryTab}.js
// STAT_GAINS → config/ClassesConfig.js, _itemStatStr → config/ItemsConfig.js
    ...
    for(let i=G.projs.length-1; i>=0; i--) {
        ...
        for(const e of enemies) {
            ...
            if(Math.hypot(f.x - ex, f.y - ey) < CFG.TILE * 0.8) {
                G.p._dealProjDamage(e, f.dmg, G.parts, G.floats);
                f.alive = false;
                for(let j=0; j<15; j++) {
                    G.parts.push(new Particle(f.x, f.y, '#e68220', -3));
                }
