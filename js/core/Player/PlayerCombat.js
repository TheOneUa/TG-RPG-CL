...
// ============================================================
// Формулы урона и крита вынесены в combat/DamageFormula.js и
// combat/CritSystem.js, обработка смерти врага — в combat/EnemyDeath.js
// (эти три файла должны грузиться до этого файла — см. index.html).
Object.assign(Player.prototype, {
    ...
    // Нанести урон врагу с крит-шансом и учётом брони (с учётом активной способности мечника)
    _dealDamage(e, parts, floats) {
        const result = computeDamage(this.effAtk, e.def, this, { forceCrit: false });
        ...
        const healed = applyLifesteal(this, dmg);
        if (healed > 0) floats.push(new FText(this.x, this.y - CFG.TILE * 1.3, '🩸+' + healed, '#ff6688', 12));
        ...
        if (e.hp <= 0) resolveEnemyDeath(e, parts, floats);
        return dmg;
    },

    // Урон по конкретной цели от снаряда (стрела/файрбол) — броня, крит, lifesteal
    _dealProjDamage(e, baseDamage, parts, floats) {
        const result = computeDamage(baseDamage, e.def, this, { noVariance: true });
        const dmg = result.damage;
        const isCrit = result.isCrit;
        ...
        e.hp -= dmg;
        floats.push(new FText(ex, ey - CFG.TILE, '-' + dmg, isCrit ? '#ff8822' : '#ff4444', isCrit ? 16 : 13));
        for (let j = 0; j < (isCrit ? 12 : 7); j++) parts.push(new Particle(ex, ey, '#e04040'));
    ...
    hurt(dmg, floats) {
        ...
        const r = applyArmor(dmg, this.effDef);
        this.hp -= r;
        this.flash = 12;
